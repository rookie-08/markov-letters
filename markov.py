...
def generate_string(markov_chain): # UNSTABLE
    """
    ...
    prob_list = get_probability_list(markov_chain, 0, "START")
    new_token = generate_token(prob_list)
    if (new_token == "END"):
        return generated_string
    generated_string = generated_string + new_token
    context_token = new_token

    prob_list = get_probability_list(markov_chain, 1, context_token)
    new_token = generate_token(prob_list)
    if (new_token == "END"):
        return generated_string
    generated_string = generated_string + new_token
    context_token = generated_string[-2] + generated_string[-1]

    while True: # Loop for the second order Markov Chain
        prob_list = get_probability_list(markov_chain, 2, context_token)
        new_token = generate_token(prob_list)
        if (new_token == "END"):
            return generated_string
        generated_string = generated_string + new_token
